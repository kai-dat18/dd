...
simulate <- function(f) {
  ...
  for (i in 1:50) {
    ir <- f()
    ir_ona <- na.omit(ir)
    sample <- dplyr::sample_n(ir_ona, 100, replace = T)
    m <- lm(Sepal.Width ~ Sepal.Length * Species, data = sample)
    residuals <- predict(m, sample) - sample$Sepal.Width
    ir_na <- ir %>% filter(is.na(Sepal.Width))
    ir_na$Sepal.Width <- predict(m, newdata = ir_na) + sample(residuals, 1)
